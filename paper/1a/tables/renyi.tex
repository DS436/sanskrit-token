...
\bottomrule
\end{tabular}
\caption{R\'enyi efficiency on S\=amayik test (prose), Sanskrit side in both script variants. The English-side rows carry the English text under the named pivot and have no script variant, so their values are placed in the right-hand pair of columns. Reported as a diagnostic only: the measure can be gamed, so it supports no claim in this paper on its own, and it is tabulated for the size-matched 32,000- and 64,000-piece arms and their pair-matched controls rather than extended over the byte-matched and 128,000-piece arms of Section~\ref{sec:rq2-controlled}.}
\label{tab:renyi}
\end{table*}
