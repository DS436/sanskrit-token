...
def _sangraha_lines(config: Mapping[str, Any], root: Path) -> Iterator[str]:
    ...
    for line in iter_sangraha_lines(
        cache_dir,
        ...
        yield normalise_line(to_slp1(line, "devanagari"))
...
def _wikipedia_lines(config: Mapping[str, Any], root: Path) -> Iterator[str]:
    ...
    for line in iter_wikipedia_lines(
...
def source_lines(key: str, config: Mapping[str, Any], root: Path) -> Iterator[str]:
    """The SLP1 lines of one named source, streaming, in corpus order."""
    if key == "dcs_train":
...
def source_provenance(key: str, config: Mapping[str, Any]) -> dict[str, Any]:
    """What a source is, for the manifest: repository, revision and licence."""
    if key == "dcs_train":
...
def build_track2(
    ...
    *,
    ...
    indices: Mapping[str, frozenset[str]],
    shingle_k: int,
) -> tuple[CorpusStats, FilterCounts]:
    ...
    for source in sources:
        ...
        for index, line in enumerate(source_lines(source, config, root), start=1):
            if line_filter.keep(line, source):
                sink.write(line)
            if index % _PROGRESS_EVERY == 0:
                logger.info(
...
def _dcs_heldout(config: Mapping[str, Any], root: Path, out_dir: Path) -> dict[str, CorpusStats]:
    """`heldout_dcs.txt` and `heldout_dcs_split.txt`, in held-out order, unfiltered.

    The held-out split *is* the evaluation set, so neither leakage layer applies to it and
    it is not deduplicated: a sentence that occurs twice in the evaluation text is two
...
def _parallel_heldout(
    entry: Mapping[str, Any], root: Path, out_dir: Path
    ...
    raw_stats = write_lines(
        out_dir / f"heldout_{name}.txt",
        (normalise_line(str(record["raw_slp1"])) for record in records),
    )
    split_stats = write_lines(
        out_dir / f"heldout_{name}_split.txt",
        (normalise_line(str(record["output"])) for record in records),
    )
    return {name: raw_stats, f"{name}_split": split_stats}
...
def build(
    ...
    *,
    ...
    heldout: dict[str, CorpusStats] = _dcs_heldout(config, resolved_root, out_dir)
    for entry in config.get("heldout_parallel", []):
        heldout.update(_parallel_heldout(dict(entry), resolved_root, out_dir))
    logger.info(
        "held-out texts: %s",
    ...
    track2_stats, track2_counts = build_track2(
        config,
        resolved_root,
        out_dir,
        excluded=excluded,
    ...
    for name, stats, counts in entries:
        ...
        entry_manifest: dict[str, Any] = {
            ...
            **asdict(stats),
            "sources": {
                source: source_provenance(source, config) for source in counts.n_in
            },
            "shingle_k": shingle_k,
