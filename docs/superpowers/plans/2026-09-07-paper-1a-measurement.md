...
- [ ] Implement; integrate; run Exp02 tests; ruff + mypy.
...
- [ ] Failing tests: `subset` preserves alignment; `tpp_from_parts` equals `tpp` on the same inputs with the same seed; bin assignment at the edges (8→bin 1, 9→bin 2, 41→last); sparse flag; figure writes both formats.
- [ ] Implement; ruff + mypy.

### Task 3: Re-run Exp02, refresh snapshot, update Exp02 README
- [ ] `uv run python experiments/02_tpp_parallel/run.py` at a clean tree; numeric diff of every pre-existing leaf against `results/02_tpp_parallel/results.json` must be zero; copy to `results/02_tpp_parallel/`; update `results/README.md` row; append the Rényi and by-length tables to `experiments/02_tpp_parallel/README.md` with two sentences of reading each.
